...
public:
    ...
    bool is_stable_;
    perception_msgs::DetectedObject object_;
    std::string label_;
    ...
    void updateMeasurementForCTRV(const std::vector<perception_msgs::DetectedObject>& object_vec);

    void uppateForCTRV();

    void updateEachMotion(const double detection_probability, const double gate_probability, const double gating_threshold,
                          const std::vector<perception_msgs::DetectedObject>& object_vec, std::vector<double>& lambda_vec);

    void updateSUKF(const std::vector<perception_msgs::DetectedObject>& object_vec);
    ...
    void update(const bool use_sukf, const double direction_probability, const double gate_probability,
                const double gating_threshold, const std::vector<perception_msgs::DetectedObject>& object_vec);

    void prediction(const bool use_sukf, const double dt);
};
