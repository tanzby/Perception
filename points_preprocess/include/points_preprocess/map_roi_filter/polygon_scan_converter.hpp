...
namespace points_preprocess {
    ...
    public:
        ...
        PolygonScanConverter() = default;
        virtual ~PolygonScanConverter() = default;
    ...
    template <typename T>
    const double PolygonScanConverter<T>::s_epsilon_ =
            std::numeric_limits<float>::epsilon();

    template <typename T>
    const double PolygonScanConverter<T>::s_inf_ = std::numeric_limits<T>::infinity();

    template <typename T>
    void PolygonScanConverter<T>::Init(const Polygon& polygon) {
        Reset();
        polygon_ = polygon;
    ...
    template <typename T>
    void PolygonScanConverter<T>::Reset() {
        polygon_ = Polygon();
        is_singular_.clear();
        segments_.clear();
        ks_.clear();
    ...
    template <typename T>
    void PolygonScanConverter<T>::ScanCvt(const T& scan_loc, DirectionMajor dir_major,
                                      std::vector<IntervalOut>* scan_intervals) {
    ...
    template <typename T>
    void PolygonScanConverter<T>::ScansCvt(
...
// note, there exits problem when input T=double
    template <typename T>
    void PolygonScanConverter<T>::DisturbPolygon(const DirectionMajor dir_major) {
    ...
    template <typename T>
    void PolygonScanConverter<T>::ParsePolygon(const DirectionMajor dir_major,
                                           const bool disturb) {
    ...
    template <typename T>
    void PolygonScanConverter<T>::BuildEt() {
    ...
    template <typename T>
    void PolygonScanConverter<T>::UpdateAet(std::vector<IntervalOut>* scan_intervals) {
    ...
    template <typename T>
    bool PolygonScanConverter<T>::ConvertSegment(const size_t seg_id,
                                             std::pair<int, Edge>* out_edge) {
        const auto& segments = segments_[dir_major_];
